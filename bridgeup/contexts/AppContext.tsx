...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const transformedQuestions: Question[] = dbQuestions.map(q => ({
        ...
        assignedMentors: [], // Will be populated from sessions
    ...
    try {
      ...
      for (const session of allSessions) {
        ...
        const transformedMessages: Message[] = (session.messages || []).map(m => ({
          ...
          senderName: m.sender_id === authUser.id ? (user?.name || 'you') : (mentorProfile.username || mentorProfile.full_name || 'advisor'),
          senderAvatar: m.sender_id === authUser.id ? (user?.avatar || '🥺') : (mentorProfile.avatar_url || '💕'),
          isAdvisor: m.sender_id !== authUser.id,
          timestamp: new Date(m.created_at),
          isMe: m.sender_id === authUser.id,
        }));

        // Count unread messages (messages from advisor that are unread)
        const unreadCount = transformedMessages.filter(m => m.isAdvisor && !m.isMe).length;
        ...
        const chat: Chat = {
          ...
          participants: [{
            id: mentorProfile.id,
            name: mentorProfile.username || mentorProfile.full_name || 'advisor',
        ...
        setQuestions(prev => prev.map(q =>
          q.id === session.question_id
            ? {
                ...q,
                assignedAdvisors: chat.participants,
                chatId: session.id,
                status: session.status === 'resolved' ? 'resolved' : 'active',
              }
            : q
  ...
  /**
   * Fetch available advisors from Supabase
   * For now, we'll just fetch users with advisor role
   */
  const fetchAvailableAdvisors = async () => {
    try {
      console.log('[AppContext] Fetching available advisors');

      // TODO: Create a proper query to get available advisors
      // For now, we'll return empty array until we implement advisor listing
      setAvailableAdvisors([]);

      console.log('[AppContext] Advisors loaded');
    } catch (error) {
      console.error('[AppContext] Error fetching advisors:', error);
    }
  };
    ...
    try {
      ...
      if (topics.length > 0) {
        ...
        setTrendingTopics([
          ...
          {
            id: 'demo-2',
            title: 'which colleges should I apply to?',
            participants: 18,
  ...
  const handleNewMessage = async (sessionId: string, dbMessage: supabaseService.Message) => {
    try {
      ...
      const newMessage: Message = {
        ...
        isAdvisor: dbMessage.sender_id !== authUser?.id,
