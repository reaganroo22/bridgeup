...
export async function sendMessage(
  ...
  try {
    console.log('[sendMessage] Sending message in session:', sessionId)
...
export async function markMessagesAsRead(
  ...
  try {
    console.log('[markMessagesAsRead] Marking messages as read for user:', userId)
...
export async function updateSubscription(
  ...
  try {
    ...
    if (session.user.id !== userId) {
      throw new Error(`User ID mismatch: auth=${session.user.id}, target=${userId}`)
    }
...
export async function syncAllMentorStats(): Promise<ServiceResponse<boolean>> {
  try {
    ...
    const { error } = await supabase.sql`
      ...
      SET 
        total_questions_answered = (
          SELECT COUNT(DISTINCT ads.question_id)
          FROM advice_sessions ads
          WHERE ads.mentor_id = mentor_profiles.user_id 
          AND ads.status = 'resolved'
        ),
        average_rating = COALESCE((
          SELECT AVG(r.rating)
          FROM ratings r
          WHERE r.mentor_id = mentor_profiles.user_id
        ), 0),
        total_helpful_votes = COALESCE((
          SELECT COUNT(*)
          FROM ratings r
          WHERE r.mentor_id = mentor_profiles.user_id
          AND r.rating >= 4
    ...
    console.log('[syncAllMentorStats] All mentor stats synced successfully')
    return { data: true, error: null }
  } catch (error) {
