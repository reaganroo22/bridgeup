...
export function UserProfileProvider({ children }: { children: React.ReactNode }) {
  ...
  const getRelevantCategories = (): string[] => {
    ...
    // Return categories that match user's interests for college admissions
    const allCategories = [
      'college-applications', 'sat-act-prep', 'essay-writing', 'college-selection', 
      'scholarships-aid', 'extracurriculars', 'interviews', 'recommendation-letters',
      'college-visits', 'early-decision-action', 'gap-year', 'community-college',
      'stress-anxiety', 'family-parents', 'career-exploration', 'academic-planning',
      'financial-aid', 'application-deadlines', 'campus-culture', 'major-selection',
      'test-prep-strategies', 'college-essays'
    ];

    // Prioritize categories based on education level
    if (isHighSchool) {
      const highSchoolPriority = ['college-applications', 'sat-act-prep', 'essay-writing', 'college-selection', 'scholarships-aid', 'stress-anxiety'];
      ...
      const collegePriority = ['recommendation-letters', 'college-visits', 'interviews', 'career-exploration', 'campus-culture', 'major-selection'];
